	...
	<body>
					...
					<c:forEach var="list" items="${list}">
						<c:if test="${list.image == null}" >
							<article class="style8">
								...
								<br/>
								<b style = "color : black">${list.content}</b>
							</article>
						</c:if>
						<c:if test="${list.image != null}">
							<article class="style8">
		...
		<c:if test="${count > 0}">
			<c:if test="${startPage>10}">
				<a href="/spot/media?pageNum=${startPage-10}" >[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
				<a href="/spot/media?pageNum=${i}" >[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < pageCount}">
				<a href="/spot/media?pageNum=${startPage+10}" >[다음]</a>
			</c:if>
		</c:if>
	</body>
</html>