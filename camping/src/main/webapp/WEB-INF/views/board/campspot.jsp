			...
			<c:forEach var="board" items="${boardList}">
				<tr height="30">
		    		...
		    		<td align="center" width="250" >  	    
		      			<a href="/board/content?boardnum=${board.boardnum}&pageNum=${currentPage}&category=1&id=${memId}">
		           			${board.title}
			  			</a> 
		          	</td>
				...
				<td align="center">
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href="/board/campspot?id=${memId}&category=1&pageNum=${startPage - 10}">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="/board/campspot?id=${memId}&category=1&pageNum=${i}">[${i}]</a>
						</c:forEach>
						<c:if test="${endPage < pageCount}">
							<a href="/board/campspot?id=${memId}&category=1&pageNum=${startPage + 10}">[다음]</a>
						</c:if>
					</c:if>
				</td>
			</tr>
